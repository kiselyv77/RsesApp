...
class Halper {
    ...
    fun Swap(adapter: AdapterVideo): ItemTouchHelper {
        return ItemTouchHelper(object : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.RIGHT){
            override fun onMove(recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder, target: RecyclerView.ViewHolder): Boolean {
                return false
            }
            override fun getSwipeDirs (recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder): Int {
                //Отключение свайпа при активных скачиваниях
                val reference: StorageReference = FirebaseStorage.getInstance().reference
                if (reference.activeDownloadTasks.size != 0) return 0
    ...
    fun refactor(item:ContentVideo, adapter:AdapterVideo){
        ...
        val radio_group_type = dialog.findViewById<RadioGroup>(R.id.radioGroupType)
        ...
        /*Проверяю какой тип был до изменения*/
        var typeCheck:Int? = null
        if("OTBR" == item.type){
            typeCheck = R.id.OTBR
        }
        else{
            typeCheck = R.id.NOT_OTBR
        }
        radio_group_type.check(typeCheck)
    ...
    fun initSearchView(searchView: SearchView, rcView: RecyclerView, list_data:ArrayList<ContentVideo>, list_data_display:ArrayList<ContentVideo>)
